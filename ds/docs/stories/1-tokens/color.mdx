...
export const renderColors = (colors) => (
	<div className="flex flex-wrap gap-px">
		{colors.map((color) => (
			...
			>
				<div className="bg-color-white-alpha-7 px-xs-1 pb-xs-0 text-color-black text-size-xs rounded-xs">
					{color}
				</div>
			</div>
		))}
		...
		<tbody>
			{semanticTokens.map(([name, token]) => (
				<tr key={name}>
					...
					<td>
						<DocsTokenThemeGrid
							lightSlot={<pre>{token.$ref.light}</pre>}
							darkSlot={<pre>{token.$ref.dark}</pre>}
						/>
					</td>
					<td>
						<DocsTokenThemeGrid
