  ...
  <div>
    ...
    <DocsCodeBlock
      code={`
				...
				//
				const App = () => {
					const { tokens, colorMode, isUiDark, isUiLight, changeColorMode } = useThemeService()
					return (
						<button
						  data-mode={colorMode}
...
| isUiDark        | `boolean`     | -                 | -      | Indicates whether the current mode is dark           |
| isUiLight       | `boolean`     | -                 | -      | Indicates whether the current mode is light          |
...
- Use `isUiDark` and `isUiLight` for conditional rendering based on current color mode
