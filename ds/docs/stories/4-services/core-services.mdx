  ...
  <div>
    ...
    <DocsCodeBlock
      code={`
				...
				//
				const App = () => {
					const { tokens, colorTheme, isUiDark, isUiLight, changeColorTheme } = useThemeService()
					return (
						<button
...
- The `cookieKey` prop enables color theme persistence across sessions
- Design token objects (`tokens.color`, `tokens.spacing`, etc.) provide type-safe access to CSS variables
- Use `isUiDark` and `isUiLight` for conditional rendering based on current color theme
- All design tokens are automatically updated when the color theme changes
