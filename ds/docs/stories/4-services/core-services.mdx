...
import { DocsColumns, DocsCodeBlock } from '@ds/docs/core'
...
<DocsColumns>
  <div>
    ...
    <DocsCodeBlock
      code={`
        import { useI18nService } from '@ds/core'
        //
        const App = () => {
...
<DocsColumns>
  <div>
    ...
    <DocsCodeBlock
      code={`
        import { useRoutingService } from '@ds/core'
        //
        const App = () => {
...
This service enables dark/light mode switching and exposes all design system tokens through CSS variables.
...
<DocsColumns>
  <div>
    ...
    <DocsCodeBlock
      code={`
        ...
        //
        const App = () => {
          const { tokens, colorMode, isDarkMode, isLightMode, changeColorMode } = useThemeService()
          return (
            <button
              data-mode={colorMode}
              style={{ color: tokens.color['black'] }}
              onClick={changeColorMode}
            >
              {isDarkMode ? 'Dark' : isLightMode ? 'Light' : '??'}
            </button>
          )
        }
      `}
  ...
  <div>
    ...
    <DocsCodeBlock
      code={`
        import { ThemeService } from '@ds/core'
        //
        const App = ({ children }) => (
          <ThemeService cookieKey="color-mode">{children}</ThemeService>
        )
      `}
    />
  </div>
...
| Property        | Type          | Params            | Return | Description                                          |
| --------------- | ------------- | ----------------- | ------ | ---------------------------------------------------- |
| colorMode       | `ColorMode`   | -                 | -      | Current color mode value (`'light'` or `'dark'`)     |
| colorTheme      | `ColorTheme`  | -                 | -      | Current color theme value (`'simple'` or `'modern'`) |
| isDarkMode      | `boolean`     | -                 | -      | Indicates whether the current mode is dark           |
| isLightMode     | `boolean`     | -                 | -      | Indicates whether the current mode is light          |
| changeColorMode | `function`    | `mode: ColorMode` | -      | Changes the color mode to the specified value        |
| tokens          | `ThemeTokens` | -                 | -      | Token collections mapped to CSS variable values      |

**Usage notes:**

- The `cookieKey` prop enables coloor mode persistence across sessions
- Design token objects (`tokens.color`, `tokens.spacing`, etc.) provide type-safe access to CSS variables
- Use `isDarkMode` and `isLightMode` for conditional rendering based on current color mode
- All design tokens are automatically updated when the coolor mode changes
